...
import numpy as np
import utils
from diagnosis_demo_cnn3 import Diagnosis
from Log import loger
from AutomaticWeightedLoss import AutomaticWeightedLoss
from data_loader3 import Loader_single,Loader_unif_sampling,Loader_unif_sampling_DA
...
def wgn(x, snr):
    ...
    npower = xpower / snr
    return torch.rand(len(x)) * torch.sqrt(npower)
...
#        D_optim = optim.Adam(self.d.parameters(),lr = self.Dlr)
        target_optim = optim.Adam([{'params':self.t_model.parameters()},
                                   {'params':self.balanceloss.parameters(),
                                    'lr':self.Flr*10,'weight_decay':0.0001}],                                    
...
#                utils.set_requires_grad(self.d,requires_grad = True)             #因为zero_grad和step只对某个优化器有效
                (x1, x2,y1,y2,d1,d2),(x1_t, x2_t,y1_t,y2_t,d1_t,d2_t) = next(batch_iterator)               #可能可以节省内存，猜的
                # if self.gpu
                xs = torch.cat((x1,x2),dim=0)
                ys = torch.cat((y1,y2),dim=0)
                ds = torch.cat((d1,d2),dim=0)
                
                xs_t = torch.cat((x1_t,x2_t),dim=0)
                ys_t = torch.cat((y1_t,y2_t),dim=0)
                ds_t = torch.cat((d1_t,d2_t),dim=0)
                ...
                x1,x2 = x1.cuda(),x2.cuda()#self.to_cuda(x1,x2,x3)
...
#                y3,_ = self.t_model(x3)#.view(xs2.size(0),-1)
                ...
                if method in ['JAN','CORAL','MMD']:
#                    def GAP(self,x):
#                        x = nn.AvgPool2d(x.size(2))(x)
#                
#                        x = x.view(x.size()[0], -1)
#                        return x                         
                    f1 = self.t_model.feature_extractor(x1)#.view(xs.size(0),-1)
                    f2 = self.t_model.feature_extractor(x2)#.view(xs2.size(0),-1)
#                    f3 = self.t_model.feature_extractor(x3)#.view(xs2.size(0),-1)
                    f1 = self.GAP(f1)
                    f2 = self.GAP(f2)
#                    f3 = self.GAP(f3)
...
#                    MMDloss3 = loss(f1,f3)
                    ...
                    Dloss = MMDloss1#+MMDloss2+MMDloss3
                    Total_loss = Closs + Dloss           
...
#experiment 2 
    ...
    SUST_speed_list = ['SUST_500.mat',
                 ...
                 'SUST_1500.mat',
                 'SUST_2000.mat',]   
    methods = ['ADGN','WLP0.1','WLP0.5','WLP1','ERM',
               'ERMwln','JAN','CORAL','MMD','CADGN','ADGNwln']
#    methods = ['JAN','CORAL','MMD','CADGN','ADGNwln']
#    methods = ['MMD','CADGN','ADGNwln']
#    methods = ['ADGN','WLP0.1','WLP0.5','WLP1','ERM',
#               'ERMwln','JAN','CORAL']
#    methods = ['ADGN','CADGN']
...
#        method = 'ERMwln'
        ...
        for j in range(5):
            for i in range(3):
                ...
                CWRU_list = ['JNU_600.mat',
                 'JNU_800.mat',
                 'JNU_1000.mat',
                   
                   ]
#                CWRU_list2 = ['SBDS_0K_10_small sample.mat',
#                 'SBDS_1K_10_small sample.mat',
#                 'SBDS_2K_10_small sample.mat',
#                 'SBDS_3K_10_small sample.mat',
#                   ]
                ...
                diag = Diagnosis(n_class=4,lr=0.001,batch_size=128,
                         gpu=True,domains=2,save_dir='save_dir/'+ex_name+'/',
        ...
        #%%     DG&DA   
                diagtl = TL_diagnosis(save_dir='save_dir/'+ex_name+'/',target_name = target_name,
                                    ...
                                    Flr = 0.0001, class_num=4,domains=2,norm=norm_type)
