...
#        acc = utils.caculate_acc(prediction,yt)
        ...
        return acc
#    def output_batch(self,t_dataset,modeldir='trained_target_model'):
#        '''
#        evaluate the performence of target domain 
#        '''
#                
...
#experiment 2 
    ...
    for method in methods:
        for i in range(4):
            ...
            ex_name = 'SUST_5_times/'+method
            
            CWRU_list = ['SUST_500.mat',
             'SUST_1000.mat',
             'SUST_1500.mat',
             'SUST_2000.mat',]
            print(os.path.exists('save_dir/'+ex_name+'/'+'SUST_500.mat.pkl'))
    ...
    #                 model_name = target_name,norm='ln')
    ...
    #%%     DG&DA   
            diagtl = TL_diagnosis(save_dir='save_dir/'+ex_name+'/',target_name = target_name,
                                batchsize = 20,Dlr = 0.0001,
                                Flr = 0.001, class_num=10,domains=3,norm='ln')
            E = diagtl.t_model.cpu().mid_rep
            Fs1 = E(s1,0,5,0).view(2000,-1)
            Fs2 = E(s2,0,5,0).view(2000,-1)
            Fs3 = E(s3,0,5,0).view(2000,-1)
            Ft = E(target,0,5,0).view(2000,-1)
...
#            print(Fs1.type)
            ...
            save_dir2 = 'save_dir/feature2/'
            
            if not os.path.exists(save_dir2):
                os.makedirs(save_dir2)
    
            io.savemat(save_dir2+str(i)+method+'SUSTfeatures.mat',
                       {'Fs1': Fs1,
                        'Fs2': Fs2,
                        'Fs3': Fs3,
                        'Ft': Ft,
                        })        
#        diagtl.load(diagtl.t_model,diagtl.target_name)
