...
class Loader_unif_sampling(data.Dataset):
    """
    加载多领域数据集，可用于加载3源域数据集
    """
    def __init__(self, mat_path1, mat_path2, mat_path3, mode='train', transform=None):
        """
        Args:
            mat_path1:源域路径1
            mat_path2:源域路径2
...
if __name__ == '__main__':
    ...
    ax, bx, cx, ay, by, cy, ad, bd, cd = source_dataset.__getitem__(100)
    print('领域a数据：ax.shape = ', ax.shape)
    print('领域a数据标签：ay.shape = ', ay.shape)
    print('领域a域标签：ad = ', ad)
    print('领域b数据：bx.shape = ', bx.shape)
    print('领域b数据标签：by.shape = ', by.shape)
    print('领域b域标签：bd = ', bd)
    print('领域c数据：cx.shape = ', cx.shape)
    print('领域c数据标签：cy.shape = ', cy.shape)
    print('领域c域标签：cd = ', cd)
