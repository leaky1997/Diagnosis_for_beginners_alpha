...
#                ratio = Floss/Dloss
#                
#                utils.lr_changer(D_optim,decayrate = 0.9)
#                utils.lr_changer(target_optim ,decayrate = 0.9)
...
#                diagtl.fit(xs_train,ys_train,xt_train,yt_train,epoches = 500)

#experiment 3
    
    BS_domains=['datasets/SBDS_0K_10.mat',
             'datasets/SBDS_0K_10.mat',
             'datasets/SBDS_0K_10.mat',
             'datasets/SBDS_0K_10.mat']
    BS_name=['SB0','SB1','SB2','SB3']
    
    CW_domains=['datasets/CWRU_0hp_10.mat',
             'datasets/CWRU_1hp_10.mat',
             'datasets/CWRU_2hp_10.mat',
             'datasets/CWRU_3hp_10.mat']
    CW_name=['CW0','CW1','CW2','CW3']
    
    
    for i in range(4):
        print('source domain is %d'%i)
        source_domain = CW_domains[i]
        source_name = CW_name[i]
        ...
        for j in range(4):
#            if i!= j:
                target_domain = BS_domains[j]
                target_name = BS_name[j]
                ...
                xs_train,ys_train,xs_test,ys_test = data_reader(source_domain)
                xt_train,yt_train,xt_test,yt_test = data_reader(target_domain)
                diagtl.fit(xs_train,ys_train,xt_train,yt_train,epoches = 1000)
